...
export default class Editor {
  ...
  public async loadAllFont(data) {
    // 如果当前对象包含 'fontFamily'，则将其添加到结果数组中
    if (data.fontFamily) {
      data.fontFamily = 'AlibabaPuHuiTi'
      await loadFont(data.fontFamily);
    }

    // 如果当前对象包含 'objects' 数组，递归地处理每个子对象
    if (data.objects && Array.isArray(data.objects)) {
      data.objects.forEach((obj) => {
          this.loadAllFont(obj)
      });
    }
  }
